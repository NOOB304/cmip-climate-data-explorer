...
class ProcessingPage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        form.addRow("检测结果", self.detected)
        form.addRow("处理变量", self.variable)
        form.addRow("输出时间尺度", self.target_period)
        form.addRow("合成规则", self.method)
        form.addRow("单位转换", self.unit_conversion)
        form.addRow("重采样", self.resampling_enabled)
        form.addRow("目标网格", self.target_grid)
        form.addRow("重采样方法", self.resampling_method)
        form.addRow("模板 TIF", self.template_panel)
        form.addRow("矢量裁剪", self.clipping_enabled)
        form.addRow("研究区文件", self.region_panel)
        root.addLayout(form)
